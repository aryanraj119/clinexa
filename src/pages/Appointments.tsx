...
const Appointments = () => {
  ...
  const fetchAppointments = async (userId: string) => {
    ...
    const doctorsResult = await localDb.from("doctors").select("*");
    const doctorMap = new Map((doctorsResult.data || []).map((d: any) => [d.id, d]));
    ...
    if (data) {
      const appointmentsWithDoctors = data.map((apt: any) => {
        const doctor: any = doctorMap.get(apt.doctor_id) as any;
        const specId = doctor?.specialization_id;
        const spec: any = specMap.get(specId) as any;
        return {
          ...apt,
          doctor: {
            ...doctor,
      ...
      const { data: reviewData } = await localDb
        .from("reviews")
