...
export default function BloodBank() {
  ...
  const fetchBloodBanks = async () => {
    ...
    if (result.data) {
      setBloodBanks(result.data as unknown as BloodBank[]);
    }
  };

  const fetchInventory = async () => {
    const bbResult = await localDb.from("blood_banks").select("*");
    const bbMap = new Map((bbResult.data || []).map((b: any) => [b.id, b]));
    ...
    if (result.data) {
      const inventoryWithBanks = result.data.map((item: any) => ({
        ...item,
        blood_banks: bbMap.get(item.blood_bank_id)
      }));
      setInventory(inventoryWithBanks as unknown as BloodInventory[]);
    }
  };
  ...
  const fetchEmergencyRequests = async () => {
    ...
    if (result.data) {
      setEmergencyRequests(result.data as unknown as EmergencyRequest[]);
    }
  };
  ...
  const handleDonationRequest = async () => {
    ...
    setLoading(true);
    const { error } = await localDb.from("donation_requests").insert({
      user_id: user.id,
